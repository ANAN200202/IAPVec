...
class ViTSAM(BaseBackbone):   #这个时mmpretrain中的sam
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:  #输入为bx3x512x512
        B = x.shape[0] #batch size
        x, patch_resolution = self.patch_embed(x) #bx1024x768, 32x32; 1024个patch，每个patch取32x32，长度为3x16x16
        x = x.view(B, patch_resolution[0], patch_resolution[1],
                   self.embed_dims)  #bx32x32x768

        if self.use_abs_pos:  #True
            ...
            resized_pos_embed = resize_pos_embed(
                ...
                num_extra_tokens=0)  #bx1024x768
            x = x + resized_pos_embed.view(1, *patch_resolution,
                                           self.embed_dims)
            x = self.drop_after_pos(x) #这里是一个dropout

        outs = []
        t = 0 #输出第t层的结果 
        mid = None
        for i, layer in enumerate(self.layers): #layer层数为12
            x = layer(x)

            if i in self.out_indices: #在第12层减少通道数
                # (B, H, W, C) -> (B, C, H, W)
                x_reshape = x.permute(0, 3, 1, 2)

                if self.out_channels > 0:  
                    x_reshape = self.channel_reduction(x_reshape)  #从768通道缩减成256通道
                outs.append(self._format_output(x_reshape)) #这个里面没有处理，只有i为11时才加入out中

            if i==t:
               mid = x.detach().clone()

        return tuple(outs), mid
