...
class CascadeRoIHead(BaseRoIHead):
    ...
    def init_mask_head(self, mask_roi_extractor: MultiConfig,
        ...
        self.mask_head = MODELS.build(mask_head) #注释部分为只输出最后一个mask时的代码
    ...
    def predict_bbox(self,
        ...
        #         stage=0, x=x, rois=rois, **kwargs)
        # cls_scores = bbox_results['cls_score'] #2000x2
        # bbox_preds = bbox_results['bbox_pred'] #2000x4
        ...
        #     else:
        #         bbox_preds = self.bbox_head[2].bbox_pred_split(
    ...
    def _refine_roi(self, x: Tuple[Tensor], rois: Tensor,
        ...
        ms_scores = []
        # self.num_stages = 1
        for stage in range(self.num_stages): #3
            bbox_results = self._bbox_forward(
                stage=stage, x=x, rois=rois, **kwargs) #4000
